...
export class CodePushUtil {
    ...
    public static copyUnassignedMembers(fromParameter: any, toParameter: any) {
        for (let key in fromParameter) {
            if (
                (<any>toParameter)[key] === undefined ||
                (<any>toParameter)[key] === null
            ) {
     ...
     */
    public static getNodeStyleCallbackFor<T>(
     ...
     * TODO: remove me
     */
    public static invokeErrorCallback(
        error: Error,
    ...
    public static logError(message: String, error?: Error): void {
        const errorMessage = `${message || ""} ${CodePushUtil.getErrorMessage(
            error
        )}`;
        const stackTrace =
            error && error.stack ? `. StackTrace: ${error.stack}` : "";

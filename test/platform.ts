...
function bootEmulatorInternal(
    platformName: string,
    restartEmulators: boolean,
    targetEmulator: string,
    checkEmulator: () => Q.Promise<string>,
    startEmulator: (targetEmulator: string) => Q.Promise<string>,
    killEmulator: () => Q.Promise<string>
): Q.Promise<string> {
    ...
    function checkEmulatorReady(): Q.Promise<string> {
        ...
        console.log(
            "Checking if " + platformName + " emulator is ready yet..."
        );
    ...
    function checkEmulatorReadyLooper(): Q.Promise<string> {
        ...
        if (emulatorReadyAttempts > emulatorMaxReadyAttempts) {
            console.log(
                platformName +
                    " emulator is not ready after " +
                    emulatorMaxReadyAttempts +
                    " attempts, abort."
            );
        ...
        setTimeout(() => {
            checkEmulatorReady().then(
                ...
                () => {
                    return checkEmulatorReadyLooper().then(
                        () => {
                            looperDeferred.resolve(undefined);
                        },
                        () => {
                            looperDeferred.reject(undefined);
                        }
                    );
                }
            );
    ...
    function startEmulatorAndLoop(): Q.Promise<string> {
        ...
        startEmulator(targetEmulator).catch((error) => {
            console.log(error);
    ...
    if (restartEmulators) {
        ...
        promise = killEmulator()
            .catch(() => {
                return null;
            })
            .then(startEmulatorAndLoop);
    } else {
        promise = checkEmulatorReady().then(
            onEmulatorReady,
            startEmulatorAndLoop
        );
    }
...
export class IOSEmulatorManager implements IEmulatorManager {
    ...
    bootEmulator(restartEmulators: boolean): Q.Promise<string> {
        function checkIOSEmulator(): Q.Promise<string> {
            ...
            return tu.TestUtil.getProcessOutput(
                "xcrun simctl getenv booted asdf"
            );
        }
        function startIOSEmulator(iOSEmulatorName: string): Q.Promise<string> {
            return tu.TestUtil.getProcessOutput(
                `xcrun instruments -w "${iOSEmulatorName}"`
            ).catch((error) => {
                return undefined; /* Always fails because we do not specify a template, which is not necessary to just start the emulator */
            });
        }
        ...
        return tu.TestUtil.readIOSEmulator().then((iOSEmulatorName: string) => {
            return bootEmulatorInternal(
                "iOS",
                restartEmulators,
                iOSEmulatorName,
                checkIOSEmulator,
        ...
        return tu.TestUtil.getProcessOutput(
            ...
            .then<string>((processListOutput) => {
                ...
                if (execResult) {
                    ...
                    return Q.reject(
                        "Could not get the running application label."
                    );
                }
            })
            .then<string>(
    ...
    resumeApplication(
        ...
        return this.launchInstalledApplication("com.apple.camera")
            .then<void>(() => {
                console.log(
                    "Waiting for " +
                        delayBeforeResumingMs +
                        "ms before resuming the test application."
                );
...
export class AndroidEmulatorManager implements IEmulatorManager {
    ...
    bootEmulator(restartEmulators: boolean): Q.Promise<string> {
        ...
        return bootEmulatorInternal(
            "Android",
            restartEmulators,
            tu.TestUtil.readAndroidEmulator(),
     ...
     */
    launchInstalledApplication(appId: string): Q.Promise<string> {
        return ProjectManager.ProjectManager.execChildProcess(
            "adb shell monkey -p " +
                appId +
     ...
     */
    prepareEmulatorForTest(appId: string): Q.Promise<string> {
        return this.endRunningApplication(appId).then(() => {
            return ProjectManager.ProjectManager.execChildProcess(
...
export class PlatformResolver {
    private static supportedPlatforms: IPlatform[] = [
        Android.getInstance(),
        IOS.getInstance(),
    ];
     ...
     */
    public static resolvePlatforms(
        cordovaPlatformNames: string[]
    ): IPlatform[] {
        var platforms: IPlatform[] = [];

        for (var i = 0; i < cordovaPlatformNames.length; i++) {
            var resolvedPlatform: IPlatform = PlatformResolver.resolvePlatform(
                cordovaPlatformNames[i]
            );
            if (resolvedPlatform) platforms.push(resolvedPlatform);
            else {
                // we could not find this platform in the list of platforms, so abort
                console.error(
                    "Unsupported platform: " + cordovaPlatformNames[i]
                );
                return undefined;
            }
        }
    ...
    public static resolvePlatform(cordovaPlatformName: string): IPlatform {
        for (var i = 0; i < this.supportedPlatforms.length; i++) {
            if (
                this.supportedPlatforms[i].getCordovaName() ===
                cordovaPlatformName
            ) {

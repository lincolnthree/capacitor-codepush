...
export class ProjectManager {
     ...
     */
    public static setupProject(
        projectDirectory: string,
        templatePath: string,
        appName: string,
        appNamespace: string,
        version: string = ProjectManager.DEFAULT_APP_VERSION
    ): Q.Promise<string> {
        ...
        return ProjectManager.execChildProcess(
            "cordova create " +
                projectDirectory +
                " " +
                appNamespace +
            ...
            ProjectManager.replaceString.bind(
                undefined,
                destinationIndexPath,
                ProjectManager.CODE_PUSH_APP_VERSION_PLACEHOLDER,
                version
            )
        );
    }
    ...
    public static setupScenario(
        ...
        var packageFile = eval(
            "(" + fs.readFileSync("./package.json", "utf8") + ")"
        );
        var pluginVersion = packageFile.version;

        console.log(
            "Setting up scenario " + jsPath + " in " + projectDirectory
        );

        // copy index html file and replace
        return (
            ProjectManager.copyFile(
                templateIndexPath,
                destinationIndexPath,
                true
            )
                .then<void>(
                    ProjectManager.replaceString.bind(
                        undefined,
                        destinationIndexPath,
                        ProjectManager.SERVER_URL_PLACEHOLDER,
                ...
                .then<void>(
                    ProjectManager.replaceString.bind(
                        undefined,
                        destinationIndexPath,
                        ProjectManager.INDEX_JS_PLACEHOLDER,
                        jsPath
                    )
                )
                .then<void>(
                ...
                .then<void>(() => {
                    return ProjectManager.copyFile(
                        templateScenarioJsPath,
                        destinationScenarioJsPath,
                        true
                    );
                })
                .then<void>(
                ...
                .then<void>(() => {
                    return ProjectManager.copyFile(
                        templateConfigXmlPath,
                        destinationConfigXmlPath,
                        true
                    );
                })
                .then<string>(
                    ProjectManager.replaceString.bind(
                        undefined,
                        destinationConfigXmlPath,
                        ProjectManager.ANDROID_KEY_PLACEHOLDER,
                ...
                .then<string>(
                    ProjectManager.replaceString.bind(
                        undefined,
                        destinationConfigXmlPath,
                        ProjectManager.IOS_KEY_PLACEHOLDER,
                ...
                .then<string>(
                    ProjectManager.replaceString.bind(
                        undefined,
                        destinationConfigXmlPath,
                        ProjectManager.PLUGIN_VERSION_PLACEHOLDER,
                        pluginVersion
                    )
                )
                .then<string>(() => {
                    return build
                        ? ProjectManager.buildPlatform(
                              projectDirectory,
                              targetPlatform
                          )
                        : ProjectManager.preparePlatform(
                              projectDirectory,
                              targetPlatform
                          );
                })
     ...
     */
    public static createUpdateArchive(
        projectDirectory: string,
        targetPlatform: platform.IPlatform,
        isDiff?: boolean
    ): Q.Promise<string> {
    ...
    public static addPlugin(
        ...
        return ProjectManager.execChildProcess("cordova plugin add " + plugin, {
            cwd: projectFolder,
        });
    }
    ...
    public static buildPlatform(
        ...
        return ProjectManager.execChildProcess(
            "cordova build " + targetPlatform.getCordovaName(),
            { cwd: projectFolder },
            false
        );
    }
    ...
    public static preparePlatform(
        ...
        return ProjectManager.execChildProcess(
            "cordova prepare " + targetPlatform.getCordovaName(),
            { cwd: projectFolder }
        );
    }
    ...
    public static launchApplication(
        ...
        return targetPlatform
            .getEmulatorManager()
            .launchInstalledApplication(appNamespace);
    }

    /**
     * Kill the test app on the given target / platform.
     */
    public static endRunningApplication(
        appNamespace: string,
        targetPlatform: platform.IPlatform
    ): Q.Promise<string> {
        console.log(
            "Ending " + appNamespace + " on " + targetPlatform.getCordovaName()
        );
        return targetPlatform
            .getEmulatorManager()
            .endRunningApplication(appNamespace);
    }

    /**
     * Prepares the emulator for a test.
     */
    public static prepareEmulatorForTest(
        appNamespace: string,
        targetPlatform: platform.IPlatform
    ): Q.Promise<string> {
        console.log(
        ...
        return targetPlatform
            .getEmulatorManager()
            .prepareEmulatorForTest(appNamespace);
    }
    ...
    public static uninstallApplication(
        ...
        return targetPlatform
            .getEmulatorManager()
            .uninstallApplication(appNamespace);
    }

    /**
     * Runs the test app on the given target / platform.
     */
    public static runPlatform(
        projectFolder: string,
        targetPlatform: platform.IPlatform,
    ...
    public static replaceString(
        ...
        console.log(
            `replacing "${regex}" with "${replacement}" in ${filePath}`
        );
        replace({
            regex: regex,
     ...
     */
    public static restartApplication(
        appNamespace: string,
        targetPlatform: platform.IPlatform
    ): Q.Promise<string> {
        console.log(
        ...
        return targetPlatform
            .getEmulatorManager()
            .restartApplication(appNamespace);
    }

    /**
     * Navigates away from the application and then navigates back to it.
     */
    public static resumeApplication(
        appNamespace: string,
        targetPlatform: platform.IPlatform,
        delayBeforeResumingMs: number = 1000
    ): Q.Promise<string> {
        console.log(
            "Resuming " +
                appNamespace +
    ...
    private static copyFile(
        ...
        try {
            ...
            writeStream.on(
                "close",
                deferred.resolve.bind(undefined, undefined)
            );
